...
void recv_task(){
  ...
  while (1) {
    ...
    if (len_in >0) {
        if (data_in[0] == start ){//&& data_in[2] != myID) {
        if (checkCheckSum(data_in,len_out)) {
          printf("Received: ");
          ESP_LOG_BUFFER_HEXDUMP(TAG_SYSTEM, data_in, len_out, ESP_LOG_INFO);
        //check against myID and change myColor if received ID is not equal to myID
           // if(data_in[2] != myID){
                myColor = data_in[1];
            //}
        }
  ...
  while(1) {
    switch((int)myColor){
      case 'R' : // Red
        ...
        break;
      case 'Y' : // Yellow but used for blue
        gpio_set_level(GREENPIN, 0);
